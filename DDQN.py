...
def optimize_model(t):
    ...
    Q_policy_argmax = torch.argmax(policy_net(next_states), dim=1)

    max_next_Q = target_net(next_states).gather(1, Q_policy_argmax.long().unsqueeze(-1)).squeeze(-1)
...
def train_model(num_frames):
    ...
    while 1:
        ...
        cum_loss = []
        while not done:
            action = select_action(torch.tensor(
                np.array(state).reshape(-1, 4, HEIGHT, WIDTH)).to(device), cumulative_frames)
...
def play_single_game(q_network: torch.nn.Module, env: gym.Env, display=False, num_episodes=1):
    ...
    for i in range(num_episodes):
        ...
        while not terminal:
            ...
            state = torch.tensor(
                np.array(state).reshape(-1, 4, HEIGHT, WIDTH)).to(device)
            with torch.no_grad():
